...
def add_app(name, module_class=None, path=None, init_params=None, global_mode=False):
    ...
    # Call lifecycle hook if app can be loaded
    try:
        # Try to load app and call smpub_on_add() hook
        app = load_app(name, global_mode)
        if hasattr(app, 'smpub_on_add') and callable(app.smpub_on_add):
            print()  # Blank line before hook output
            app.smpub_on_add()
    except Exception:
        # Silently ignore errors - registration succeeded, hook is optional
        pass
